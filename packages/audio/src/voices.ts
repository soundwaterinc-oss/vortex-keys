...
export const MODELS: Record<SoundModelId, SoundModel> = {
  ...
  hammond: {
    ...
    voice: (m, f, v, b) => {
      ...
      return {
        ...
        partialRatio: 3,
        partialGain: 0.25 * lerp(0.2, 1, m.decay) * (0.5 + 0.5 * v),
        fmRatio: 1,
        fmIndex: 0,
        noiseGain: 0.05 + m.air * 0.35,
        noiseFilterHz: 4500,
        noiseQ: 0.8,
        noiseDecay: 0.012,
        attack: 0.004,
        ...
        sustain: 1,
        release: 0.03,
        cutoffHz: cap(lerp(3000, 9000, m.color) + f * 2),
        cutoffEnvHz: 0,
        filterQ: 0.5,
        lfoHz: 6.2,
  ...
  pipe: {
    ...
    voice: (m, f, v, b) => {
      ...
      return {
        ...
        cutoffHz: cap(lerp(2000, 8000, m.color) + f * 3),
        cutoffEnvHz: 0,
        filterQ: 0.4,
        lfoHz: 4.8,
        lfoPitchCents: m.motion * 6,
  ...
  voice: {
    ...
    voice: (m, f, _v, b) => {
      ...
      return {
        ...
        cutoffHz: cap(lerp(2500, 6000, m.body * 0.3 + b * 0.7)),
        cutoffEnvHz: 0,
        filterQ: 0.5,
  ...
  rhodes: {
    ...
    voice: (m, f, v, b) => ({
      ...
      partialGain: lerp(0.1, 0.4, m.color) * (0.3 + 0.7 * v),
      fmRatio: 1,
      fmIndex: lerp(0.15, 1.6, m.color) * Math.pow(v, 1.3) * (0.7 + 0.3 * b),
      ...
      cutoffHz: cap(lerp(1200, 6000, m.color) + f * 2),
      cutoffEnvHz: 2500 * v,
      filterQ: 0.6,
